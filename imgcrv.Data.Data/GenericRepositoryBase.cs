...
using imgcrv.Data.DataContracts.Exceptions;
...
namespace imgcrv.Data.Data
{
        ...
        where TId : struct
    {
        ...
        public IQueryOver<TEntity, TEntity> AsQueryOver<TEntity>(Expression<Func<TEntity>> alias = null)
        ...
        public virtual TEntity First<TEntity>(Expression<Func<TEntity, bool>> filter)
            where TEntity : class, IEntity<TId>
        {
        ...
        public virtual TEntity FirstOrDefault<TEntity>(Expression<Func<TEntity, bool>> filter)
            where TEntity : class, IEntity<TId>
        {
            return AsQueryable<TEntity>().Where(filter).FirstOrDefault();
        }

        public virtual IQueryable<TEntity> AsQueryable<TEntity>(Expression<Func<TEntity, bool>> filter)
            where TEntity : class, IEntity<TId>
        {
